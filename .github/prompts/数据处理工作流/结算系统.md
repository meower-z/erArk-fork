...
普通效果函数必须使用以下五个必填参数。二段效果使用独立的注册表和接口。

调用方在每次调用前读取执行者当时的目标，目标切换效果可以影响后续效果。目标参数没有默认值，0表示玩家。自身效果可以忽略目标参数。新效果应使用传入的目标；部分旧效果仍依赖角色当前交互对象及下层隐式读取，需逐条迁移后才能保证任意指定目标的完整结算。

统一签名：

```python
def handle_xxx(
    character_id: int,          # 执行者ID
