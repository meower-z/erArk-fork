    ...
    def change_ban_list_cmd(self, npc_id):
        ...
        if npc_id in cache.forbidden_npc_id:
            ...
            if npc_id in cache.npc_id_got:
                default.handle_chara_off_line(npc_id, cache.character_data[npc_id].target_character_id, 1, change_data = game_type.CharacterStatusChange(), now_time = cache.game_time)
                # 输出提示信息
                now_draw = draw.NormalDraw()
