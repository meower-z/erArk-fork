# Fable 5 final PR draft record — maintainer `SPECIAL_FLAG` candidate
...
Exit status: `0`; elapsed time: `52.9s`.
...
只读、最终 PR 草稿生成：不得修改文件、Git、GitHub 或 PR。请实际读取候选 diff /home/ubuntu/games/erArk-pr-discovery-settlement-special-flag（base 58587deac62149d80c82b5a3c98ad29f51cfe2b4，head c75b3b1737f5ab958b520e568d8aead59cd1d413）、写作规则，以及现有最终稿 /home/ubuntu/games/erArk/openspec/changes/fix-discovered-reaction-settlement/fable-maintainer-special-flag-pr-draft-final-20260715.md。

输出可直接用于 PR #218 的 title/body。必须与该最终稿的标题和正文逐字一致：它已经通过外科式审查，只删除了独立非目标清单。不要添加说明、不要改任何文字、不要写文件。仅输出：
TITLE:
<标题>
...
```text
已核对：候选 diff（4 个文件，+21/−1）与最终稿描述一致——`SPECIAL_FLAG` 新增 `see_h_reaction_settled` 字段、面板各反应分支就地结算并置标记、目击H状态机回写标记、NPC 外层按 `WAIT`/`MOVE` 跳过或照常结算并清除标记。以下为逐字输出的最终稿：
