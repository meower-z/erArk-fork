...
> **Historical exploration, superseded.** This document correctly preserves the direct-pain bypass as part of the bug, but its preferred canonical-state-23 helper and continuous-repetition decision were not the final submitted design. The current PR retains `route_pain_delta` and removes its genuine second `continuous_adjust` application. See `pr-212-session-closure-20260715.md`.
...
3. **Preferred: one conversion-attempt helper that delegates to canonical state 23:** accept an already source-adjusted signed pain delta plus change records; return `False` for inactive/non-positive values; for active positive values call `base_chara_state_common_settle(..., state_id=23, ability_level=ability[36], tenths_add=False, ...)` exactly once and return `True`. The common state-17 branch and each of the four direct positive writers call it. Callers retain their original state-17 write only when it returns `False`; a `True` result must skip that write so the same delta cannot become both pain and pleasure, even when canonical state 23 suppresses psychological settlement during sleep or unconsciousness. This helper does not calculate psychological ability coefficients, so that coefficient remains exactly once in the canonical state-23 owner. Its Chinese function documentation must state that callers pass the delta after their source-specific pain calculation. `handle_extra_orgasm` uses the same Boolean to select `心理快感和恐怖` versus the original `苦痛和恐怖` text, but must continue terror settlement and clear `extra_orgasm_count` in both cases.
...
- Active positive common pain forwards both `change_data` and `change_data_to_target_change`; both record only state 23.
...
- During sleep and unconsciousness, an active positive direct writer remains handled by the conversion helper even when canonical state 23 returns early: neither state 17 nor state 23 nor either change record is modified.
...
- Extra orgasm selects psychological-versus-pain text from the helper result while preserving terror settlement and counter reset.
